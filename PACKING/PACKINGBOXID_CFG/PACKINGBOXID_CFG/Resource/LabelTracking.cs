...
namespace PACKINGBOXID_CFG.Resources
{
    ...
    public class LabelTracking
    {
        ...
        public void substringPlus(ref string input,int start,int len)
        {
        ...
        public async Task<bool> doLabelTracking(string LabelName, int PathIndex, LabelManager2.Application LabApp, Document doc)
        {
            ...
            if (labeStatus != "11")
            {
                LastError = "Label file not confirmed!" + (labeStatus.Length == 2 && !(labeStatus.Substring(1, 1) != "1") ? "Need PQE Confirm" : " Need PE Confirm") + "\n\rCurrentFileName=" + LabelName + "\n\rMd5=" + wipLabelMd5 + "\n\rpathIndex=" + PathIndex + "\n\rLabel Flag=" + labeStatus + "\n\r";
                return false;
            }
            ...
            foreach (DataRow row1 in (InternalDataCollectionBase)dt1.Rows)
            {
                ...
                foreach (DataRow row2 in (InternalDataCollectionBase)dataTable4.Rows)
                {
                    ...
                    {
                        //if (_obName.Substring(0, 7) == "Barcode" && _pVarname.Substring(0, 7) == "Barcode")
                        if (_obName.ToUpper().StartsWith("BARCODE") && _pVarname.ToUpper().StartsWith("BARCODE"))
                        {
                        ...
                        if (_obName.ToUpper().StartsWith("TEXT") && _pVarname.ToUpper().StartsWith("TEXT") && _pVareg.Replace("=", "") != _obtype)
