    ...
    public class LabelTracking1
    {
        ...
        public async Task<bool> doLabelTracking_AddParamValue(string LabelName, int PathIndex, DataTable paramTable, Document doc, DataTable dtbVarName, bool isShowErrorMessage = false)
        {
            ...
            foreach (DataRow labelRow in dtbVarName.Rows)
            {
                ...
                if (_obValue.StartsWith("#"))
                {
                    ...
                    LastErrorVN += _obName + "=" + _obValue + ":Lỗi ký tự đầu tiên,không thẻ bắt đầu với ký tự '#'" + Environment.NewLine;
                }
                try
                {
        ...
        public async Task<string> getWipLabelMD5(string LabelName, int PathIndex)
        {
            try
            {
                var result = await MainWindow._sfcHttpClient.QuerySingleAsync(new QuerySingleParameterModel
                {
                    CommandText = "select md5 from sfis1.c_label_wip_t where label_name =:LABEL and path_index =:IDEX ",
                    SfcCommandType = SfcCommandType.Text,
                    SfcParameters = new List<SfcParameter>
                    {
                        new SfcParameter{ Name = "LABEL", Value = LabelName },
                        new SfcParameter{ Name = "IDEX", Value = PathIndex }
                    }
                });

                //var result = await MainWindow._sfcHttpClient.QuerySingleAsync(new QuerySingleParameterModel
                //{
                //    CommandText = "select md5 from sfis1.c_label_wip_t where label_name ='" + LabelName + "' AND path_index =6 ",
                //    SfcCommandType = SfcCommandType.Text,
                //    SfcParameters = new List<SfcParameter>
                //{
                //    new SfcParameter{ Name = "LABEL", Value = LabelName}  //PasswordBox.Password
                //}
                //});
        ...
        public async Task<string> getLabeStatus(string LabelName, int PathIndex)
        {
            ...
            if (PathIndex != 6)
            {
                mysql = "select 1*10+NVL(IPQC,'0') bb  from sfis1.c_label_wip_t where label_name ='" + LabelName.Trim() + "' and path_index=" + PathIndex;
            }
            else
            {   //pathindex=6 must PD confirm!
                mysql = "select NVL(PD,'0')*10+NVL(IPQC,'0') bb from sfis1.c_label_wip_t where label_name ='" + LabelName.Trim() + "' and path_index=" + PathIndex;
            }
            var result = await MainWindow._sfcHttpClient.QuerySingleAsync(new QuerySingleParameterModel
            {
                CommandText = mysql,
