...
namespace CodeSoft_9Codes.Resources
{
    ...
    public class LabelTracking
    {
        ...
        public async Task<bool> doLabelTracking_AddParamValue(string LabelName , int PathIndex, DataTable paramTable, Document doc, DataTable dtbVarName, DataTable dtPqe,string sPercent,string sLabelStatus, bool isShowErrorMessage = false)
        {
            DAL fDal = new DAL();
            ...
            try
            {
                //check PQE setup not enough varial
                DataTable dtbPQE = new DataTable();
                dtbPQE = dtPqe;//getPQESetup(LabelName, PathIndex); //2
                int totalValue = dtbVarName.Rows.Count;
                int totalPQESetup = dtbPQE.Rows.Count;
                int perCent = int.Parse(sPercent); //3 80
                int setup = (int)Math.Floor(((double)totalPQESetup / (double)totalValue) * 100);
                if (setup < perCent)
                {
                    DataTable dtbNotSetup = DatatableMinus(dtbVarName, dtbPQE, "VAR_NAME");
            ...
            foreach (DataRow labelRow in dtbVarName.Rows)
            {
                ...
                try
                {
                    dt = await fDal.ExcuteSelectSQL(string.Format("SELECT * FROM SFIS1.C_LABEL_CONFIG_T WHERE UPPER(LABEL_NAME) = '{0}' AND PATH_NAME={1} AND VAR_NAME='{2}'", LabelName.Trim().ToUpper(), PathIndex, _obName), sfcHttpClient);

                    if(dt.Rows.Count > 0)
                    {
                        _cValue = dt.Rows[0]["web"].ToString();
                        _cType = dt.Rows[0]["flag"].ToString();
                        switch (_cType)
                        {
                            case "LENGTH":
                                //Độ dài biến
                            ...
                            case "FIXED":
                                ...
                                break;
                            case "FIXED POSITION":
                                _cFromPos = Int16.Parse(dt.Rows[0]["FROM_POSITION"].ToString());
                                _cToPos = Int16.Parse(dt.Rows[0]["TO_POSITION"].ToString());
                                if (_obValue.Length < _cValue.Length || !_cValue.Equals(_obValue.Substring(_cFromPos, _cToPos - _cFromPos + 1)))
                                {
                                    LastErrorEng += string.Format(errorFixPositionEng, _obName, _cValue, _cFromPos, _cToPos, _obValue.Replace(" ", "<space>")) + Environment.NewLine;
                                    LastErrorVN += string.Format(errorFixPositionVN, _obName, _cValue, _cFromPos, _cToPos, _obValue.Replace(" ", "<dấu cách>")) + Environment.NewLine;
        ...
        public async Task< DataTable> getPQESetup(string LabelName, int PathIndex)
        {
            DataTable dt = new DataTable();
            DAL fdal = new DAL();
            dt = await fdal.ExcuteSelectSQL("SELECT DISTINCT VAR_NAME FROM SFIS1.C_LABEL_CONFIG_T WHERE UPPER(LABEL_NAME)='" + LabelName.ToUpper().Trim() + "' AND PATH_NAME=" + PathIndex, sfcHttpClient);
