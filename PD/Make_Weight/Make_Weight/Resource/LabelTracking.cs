...
namespace Make_Weight.Resource
{
    ...
    public class LabelTracking
    {
        ...
        public LabelTracking(MainWindow form)
        {
            this.MyForm = form;
        }
        ...
        /// <returns></returns>
        public async Task<bool> doMD5Label(string LabelName, int PathIndex, bool isShowErrorMessage = false)
        {
            string wipMD5 = await getWipLabelMD5(LabelName, PathIndex);
            if (string.IsNullOrEmpty(wipMD5))
            {
                ErrorCode = LabelTrackingErrorCode.Error_MD5;
                if (isShowErrorMessage)
                {
                    setMessage(string.Format("Not found MD5 of label file in db. " + Environment.NewLine
                    + "File name: {0}, PathIndex: {1}" + Environment.NewLine + "Table: C_LABEL_WIP_T", LabelName.ToLower(), PathIndex) + "|" + string.Format("Không tìm thấy dữ liệu MD5 của file trên server." + Environment.NewLine
            ...
            if (!wipMD5.Equals(CurrentFileMD5))
            {
                ...
                if (isShowErrorMessage)
                {
                    setMessage(string.Format("Label file Version error. " + Environment.NewLine
                    + "File name: {0}, PathIndex: {1}" + Environment.NewLine + "ServerMD5: {2} <> CurrentMD5: {3}"
                     , LabelName.ToLower(), PathIndex, wipMD5, CurrentFileMD5) + "|" + string.Format("MD5 của file label hiện tại khác với file trên server." + Environment.NewLine
                    + "File name: {0}, PathIndex: {1}" + Environment.NewLine + "ServerMD5: {2} <> CurrentMD5: {3}"
        ...
        public async Task<bool> doLabelTracking_AddParamValue_1(Document doc, string LabelName, int PathIndex, DataTable paramTable, bool isShowErrorMessage = false)
        {
            ...
            string LastErrorEng = "";
            string LastErrorVN = "";
            string flag = await getLabeStatus(LabelName, PathIndex);
            if (flag != "11")
            {
                string errstr = "";
                ...
                if (isShowErrorMessage)
                {
                    setMessage(LastErrorEng + "|" + LastErrorVN);
                }
                return false;
            }
            ...
            try
            {
                ...
                dtbPQE = await getPQESetup(LabelName, PathIndex);
                int totalValue = dtbVarName.Rows.Count;
                int totalPQESetup = dtbPQE.Rows.Count;
                int perCent = int.Parse(await getLabelSetupPercent());
        ...
        public async Task<bool> doLabelTracking_AddParamValue(string LabelName, int PathIndex, DataTable paramTable, Document doc, DataTable dtbVarName, bool isShowErrorMessage = false)
        {
            ...
            foreach (DataRow labelRow in dtbVarName.Rows)
            {
                ...
                {
                    LastErrorEng += _obName + "=" + _obValue + ":Error first character,can not start with character '#'" + Environment.NewLine;
                    LastErrorVN  += _obName + "=" + _obValue + ":Lỗi ký tự đầu tiên,không thể bắt đầu với ký tự '#'" + Environment.NewLine;
                }
        ...
        {
            if (MyForm.VI.IsChecked == true)
            {
                this.MyForm.txtError.Text = ms.Split('|')[1];
            }
            else this.MyForm.txtError.Text = ms.Split('|')[0];
        }
    }
